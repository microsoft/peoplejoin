...
from async_collab.orchestrator.orchestrators.event_reactive_func_calling.reactive_promptbuilder import (
...
class FuncCallingReactiveOrchestrator(Orchestrator):
    ...
    def call_llm_with_functions(self, messages, functions_schema):
        ...
        response = self.llm_client.send_request(
            model=str(self.llm_client.default_model),
            request=request,
        )
    ...
    def run_loop(self) -> str:
        ...
        for _ in range(max_iter):
            ...
            # Handle both OpenAI response objects and substrate response dictionaries
            if isinstance(response, dict):
                # Substrate client response format
                choice = response["choices"][0]
                message = choice["message"]
                
                # Check for function call in dict format
                if "function_call" in message and message["function_call"]:
                    func_call = message["function_call"]
                    tool_name = func_call["name"]
                    try:
                        arguments = {}
                        if "arguments" in func_call and func_call["arguments"]:
                            import json
                    ...
                    # No function call, add assistant message and break
            ...
            if isinstance(response, dict):
                # For substrate responses, construct the message
                messages.append({
                    "role": "assistant",
                    "content": None,
                    "function_call": {
                ...
                # For OpenAI responses, use the message object
                messages.append({
                    "role": "assistant", 
                    "content": message.content,
                    "function_call": {
